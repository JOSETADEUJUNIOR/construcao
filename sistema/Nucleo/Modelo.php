...
abstract class Modelo
{
     ...
     * @return $this
     */
    public function busca(?string $termos = null, ?string $parametros = null, string $colunas = '*', ?string $innerJoin = null): object
    {
        if ($innerJoin) {
            $this->query = "SELECT {$colunas} FROM {$this->schema}.{$this->tabela} INNER JOIN {$innerJoin} WHERE {$termos} ";
...
{
    if (empty($this->query)) {
        throw new \Exception('A consulta SQL não pode ser vazia.');
    }
    ...
    public function totalHoras(): float
    {
        ...
        // Iterar sobre os resultados e somar as horas
        while ($row = $stmt->fetch(\PDO::FETCH_ASSOC)) {
            // Supondo que a coluna 'horas_trabalhadas' contenha o número de horas trabalhadas para cada registro
            $totalHoras += $row['horas_trabalhadas'];
        }

        return $totalHoras;
    }

    public function valorTotalHoras(): float
{
    $stmt = Conexao::getInstancia()->prepare($this->query);
    $stmt->execute($this->parametros);
    ...
    // Supondo que 'horas_trabalhadas' e 'valor_da_hora' sejam colunas no seu banco de dados
    while ($row = $stmt->fetch(\PDO::FETCH_ASSOC)) {
        // Supondo que 'horas_trabalhadas' e 'valor_da_hora' sejam colunas no seu banco de dados
        $valorTotalHoras += $row['horas_trabalhadas'] * $row['valor_hora'];
    }
...
public function valorTotal(): float
{
    ...
    while ($row = $stmt->fetch(\PDO::FETCH_ASSOC)) {
        // Supondo que 'horas_trabalhadas' e 'valor_da_hora' sejam colunas no seu banco de dados
        $valorTotal += $row['valor_total_venda'];
    }
